...
以下文件**不受 GitHub 管理**,由用户在本地维护:

### 核心数据文件

| 文件 | 格式 | 用途 |
|------|------|------|
| `macro_final_v11.xlsx` | Excel | 118 指标打分 + 29 节点得分 + 7 链路信号流(**核心**) |
...
这些文件通过仓库根目录的 `.gitignore` 显式排除,确保 `git add .` 时不会误入。

**例外**:`samples/` 目录里保留了**一份产出样例**(2026-04-19 版),供 AI 接手时参考"正常输出长什么样"。这份样例不跟随每日产出更新。
