...
export function normalizeChinaDisplayData(source, meta = {}) {
  ...
  return {
    meta: {
      project: 'display_html_indicator_explorer',
...
async function main() {
  ...
  const outputFile = resolve(repoRoot, 'products/display_html/indicator_explorer/data/china_display_data.json');
  const sparkTargetDir = resolve(repoRoot, 'products/display_html/indicator_explorer/data/sparklines');
  const workbookFile = 'products/display_html/indicator_explorer/data/macro_final_v11.2_2_safe_formula_fix_2026-05-07.xlsx';
  const source = JSON.parse(await readFile(inputFile, 'utf8'));
