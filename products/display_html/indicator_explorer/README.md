...
本模块属于 `products/display_html/` 展示 HTML 产品线,用于验证“指标总库保留、展示层按需选择”的 HTML 方案。

它不是日报最终产物,而是日报 PPT 的前置观察工具:先看指标历史变化,再判断哪些指标值得进入日报 PPT。
...
- 历史序列来源:测试 Excel 的 `宏观数据`、`指数走势`、`A股成交额换手率测试1` Sheet,按不同表格结构抽取真实历史点
- 生成脚本:`src/build_china_display_data.mjs`

历史序列当前覆盖:

- 月度/利率/流动性指标:`宏观数据`
- A股、港股指数、利润同比、预测 PE:`指数走势`
- A股成交额、北向成交、两融余额、南向资金:`A股成交额换手率测试1`

## Demo 功能

- 左侧选择大类、分组和指标。
- 指标卡片使用 SVG 折线图展示历史序列。
- 时间区间支持 `近3期`、`近6期`、`近12期`、`全部`;日度指标按最近交易日切换,月度指标按最近月份切换。
...
- 本模块和全量展示 HTML 同属展示 HTML 产品线,但不替代全量展示页。
